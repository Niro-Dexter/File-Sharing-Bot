...
async def channel_post(client: Client, message: Message):
    reply_text = await message.reply_text("මඳක් රැඳී සිටින්න...!", quote = True)
    try:
        ...
        print(e)
        await reply_text.edit_text("කුමක් හෝ වරදක් සිදු වී ඇත..!")
        return
    ...
    reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("URL එක Share කරන්න 🔗", url=f'https://telegram.me/share/url?url={link}')]])

    await reply_text.edit(f"<b>මෙන්න ඔයාගෙ Link එක 👇</b>\n\n{link}", reply_markup=reply_markup, disable_web_page_preview = True)
